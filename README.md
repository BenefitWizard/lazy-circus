...
The demo first runs a standalone **POML TemplateHaskell** preamble (`Example.PomlDemo.runPomlDemo`) that exercises the `makePoml` macro against the templates in `app/example/prompts/` (`hello.poml`, `greeting.poml`, `contact.poml`) and prints the rendered prompt text. It needs no external services.

It then connects to PostgreSQL, runs eight scenarios sequentially, and prints the result of each:
