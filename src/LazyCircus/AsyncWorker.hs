...
runAsyncWorker :: (HasScheduledActions sc env, HasLogFunc env) => (ScenarioProgram sc () -> RIO env ()) -> RIO env ()
...
scheduleAsyncAction :: (HasScheduledActions sc env, MonadIO m, MonadReader env m) => ScenarioProgram sc () -> m ()
