...
-- update-driver ('BotHandler.runUpdate') under the test performer
-- ('LazyCircus.Testing.Performer.runWithMocks') instead of the production
...
-- performer is the test performer, so every Telegram side effect is mocked and
-- captured in the shared mailbox.
buildDemoAction :: DefaultApp AllServices -> Mocks AllServices -> IO (Update -> IO ())
buildDemoAction app mocks = do
    store <- newChatStateStore
    pure $ runUpdate (runWithMocks app mocks . runScenarioProgram) demoHandlerConfig store
